...
window.salvarPedidos = function () {
  localStorage.removeItem("pedidosClickBus");
  localStorage.removeItem("pedidos");
  localStorage.removeItem("pedidosCliente");
  localStorage.removeItem("listaPedidos");
};

window.renderizarPedidos = async function () {
  const listaPedidos = document.getElementById("listaPedidos");
  ...
  if (!admin) {
    pedidosVisiveis = pedidos.filter((pedido) => {
      return pedido.email === usuarioLogado;
    });
  }
  ...
  if (admin) {
    ...
    if (filtro === "hoje") {
      pedidosVisiveis = pedidosVisiveis.filter((pedido) => {
        return obterDataLocalPedido(pedido.data) === hoje;
      });
    }
    ...
    if (filtro === "data") {
      ...
      pedidosVisiveis = pedidosVisiveis.filter((pedido) => {
        return obterDataLocalPedido(pedido.data) === dataFiltro;
      });
    }
  ...
  listaPedidos.innerHTML = pedidosVisiveis
    .map((pedido) => {
      const numeroPedido = obterNumeroPedidoDoDia(pedido, pedidos);
      ...
      return `
        <article class="pedido-card">
          ...
          <span class="status-pedido ${pegarClasseStatus(pedido.status)}">
            ${pedido.status}
          </span>

          <button type="button" onclick="irParaDetalhePedido('${pedido.id}')">
            🔍 Detalhe do pedido
          </button>

          ${
            pedido.status !== "Cancelado" && pedido.status !== "Entregue"
              ? `<button type="button" class="btn-cancelar" onclick="cancelarPedido('${pedido.id}')">Cancelar pedido</button>`
              : ""
          }
        </article>
      `;
